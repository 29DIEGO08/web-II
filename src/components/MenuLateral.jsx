...
export default function MenuLateral() {
  ...
  const buttonStyle = ({ isActive }) =>
    `block w-48 text-lg py-3 mb-4 rounded-xl font-semibold transition text-center border
    ${isActive
      ? "border-blue-500 bg-gradient-to-r from-blue-200 via-purple-200 to-pink-200 shadow-lg scale-105 text-blue-900"
      : "border-gray-200 bg-white hover:bg-gradient-to-r hover:from-blue-50 hover:via-purple-50 hover:to-pink-50 hover:shadow-xl hover:scale-105 hover:border-blue-400 text-gray-700"
    }`;
  ...
  return (
    <aside className="w-72 min-h-screen border-r border-blue-100 flex flex-col items-center pt-12 bg-gradient-to-b from-blue-50 via-purple-50 to-pink-50">
      <div className="flex flex-col items-center mb-8">
        <img
          ...
          className="w-32 h-32 rounded-full object-cover border-4 border-blue-300 shadow-xl mb-4"
        />
        <span className="text-lg font-semibold text-gray-600">¡Bienvenido!</span>
        <span className="font-bold text-blue-600">{user?.username}</span>
      </div>
      <NavLink
        to="/"
        className="mb-10 text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 hover:scale-105 hover:drop-shadow transition"
      >
        Inicio
      </NavLink>
      {user?.role === "user" && (
        <>
          <NavLink to="/participaciones" className={buttonStyle}>🎟️ Participaciones</NavLink>
          <NavLink to="/acercade" className={buttonStyle}>ℹ️ Acerca de</NavLink>
        </>
      )}
      {user?.role === "admin" && (
        <>
          <NavLink to="/admin/crear-evento" className={buttonStyle}>🗓️ Crear Evento</NavLink>
          <NavLink to="/admin/crear-admin" className={buttonStyle}>👤 Crear admin</NavLink>
          <NavLink to="/admin/peticiones" className={buttonStyle}>📬 Peticiones</NavLink>
          <NavLink to="/acercade" className={buttonStyle}>ℹ️ Acerca de</NavLink>
        </>
      )}
      <button
        onClick={handleLogout}
        className="mt-auto mb-14 px-8 py-3 bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 hover:from-red-500 hover:via-pink-500 hover:to-purple-500 text-white rounded-xl shadow-xl font-bold text-lg transition-all duration-150 hover:scale-105"
      >
        ⏏️ Cerrar sesión
      </button>
    </aside>
